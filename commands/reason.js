...
exports.run = async (client, message, args) => {
  ...
  await modlog.fetchMessages({limit:100}).then((messages) => {
    ...
    modlog.fetchMessage(caseLog.id).then(logMsg => {
      ...
      embedSan(embed);
      embed.description = embed.description.replace(`En attente d'ajout. Utilisez ${settings.prefix}reason ${caseNumber} <raison>.`, newReason);
      logMsg.edit({embed});
    });
  });
};
...
exports.help = {
  name: 'reason',
  description: 'Met à jour la raison d\'une action effectuée par un modérateur.',
